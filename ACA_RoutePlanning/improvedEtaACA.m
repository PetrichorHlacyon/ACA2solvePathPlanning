function [dist_best, path_best, per_iter_best,per_iter_avg] = improvedEtaACA(map, start_pos_ind, goal_pos_ind)
%普通的蚁群算法没有经过改进
%改进了启发式信息
...
%         dist_matrix(i,j) = gap_size.*norm(goal_pos_sub - [i,j]);
        if dist_matrix(i,j) == 0 %终点
            dist_matrix(i,j) = 0.01;
        elseif dist_matrix(i,j) == inf %起点
            dist_matrix(i,j) = 60;
        end
    end
end
...
while Iteration_cur <= Iteration
    ...
    for i = 1:ant_numb
        ...
        while tabu(end) ~= goal_pos_ind && ...
            ...
            %                 break
            %             end 
            if sum(P_cur2allow == inf) ==1
                index_inf = find(P_cur2allow == inf);
                P_cur2allow(index_inf) = 1;
            end
    ...
    %cal avg
    if ~isnan(mean(dist_cur(index_find_goal)))
        per_iter_avg(Iteration_cur) = mean(dist_cur(index_find_goal));
    elseif Iteration_cur-1 ~= 0
        per_iter_avg(Iteration_cur) = per_iter_avg(Iteration_cur-1);
    end
    %update tao
    ...
    delta = abs(sinc(nu*Iteration_cur/Iteration).^mu);
%     2控制有几个峰 ,阶级控制峰值，控制平滑性
    eta = delta.*(eta);
